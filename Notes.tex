...
\begin{definition}
Manifold\index{manifold}, submanifold\index{manifold!submanifold}.
\end{definition}

\begin{definition}
Tangent bundle\index{tangent bundle} and tangent vector\index{tangent bundle!tangent vector} of a submanifold.
\end{definition}
...
extension, which is in Guillemin & Pollack (in hopefully enough generality).]]
...
\end{CD}
$$
The cardinality $\# P$ is to be taken as mod-two unless $M$ and $W$ are oriented, in which case $\Delta^n$ is given the  orientation with say the differences between vertices labeled by $1, \cdots, n$ and vertex $0$ (or the barycenter) serving as basis vectors, and $P$ is then oriented accordingly.
\end{definition}
...
In de Rham theory, Stokes' theorem shows that the exterior differential agrees with coboundary.  We have a similar result in this setting, which 
immediately implies that preimage cochains of manifolds without boundary are cocycles.  First we recall some basic facts about preimage
submanifolds.  The following is the Theorem on page 60 of \cite{guillemin2010differential}.

\begin{lemma}
Let $M$ be a manifold with boundary and $P$ a submanifold without boundary of $N$.  If both $f : M \to N$ and its restriction
to $\partial M$ are transverse l to $P$ 
then $f^{-1} (P)$ is a submanifold whose boundary is its intersection with $\partial M$.
\end{lemma}

Similarly we have the following.

\begin{lemma}
Let $M$ be a manifold without boundary and $P$ a submanifold with boundary of $N$.  If $f: M \to N$ is transverse to $P$ and $\partial P$
then $f^{-1}(P)$ is a submanifold with boundary given by $f^{-1} (\partial P)$.
\end{lemma}

\begin{proof}[Sketch]
Standard transversality takes care of preimage of the interior of $P$.  On the boundary, in local coordinates $P$ is given as a subspace of the
form $(x_1, \ldots, x_\ell, 0, \ldots, 0)$ with $x_\ell > 0$.  Thus the preimage of $P$ extends and on this extension one can apply
the Lemma on page 62 of \cite{guillemin2010differential} to get that the preimage of $P$ is a manifold with boundary.
\end{proof}
...
Like the proof that degree is homotopy invariant, this follows from the classification of one-manifolds.  Consider $\sigma : \Delta^{n+1} \to M$.  
Look at pull back of $W$ through $\sigma$ to get a one-manifold with boundary, by lemmas above.  
Some boundary points are on $\partial \Delta^{n+1}$ - 
