...
We  state a simplified version of the fundamental theorem of intersection theory\index{intersection theory!fundamental theorem of}.
...
%Note, in particular, that if $N$ is a submanifold of $M$ that $N$ and $M$ do not need to have the same dimension, and it is 
%generally more  interesting when they do not: it is a fact that if $N$ is a compact submanifold of a connected manifold $M$ 
%with the same dimension, then $N=M$. 
...
%linear map $df_x\colon T_xM\rightarrow T_{f(x)}M$ which in a neighborhood of $x$. This map is defined as follows: Suppose $f(x)=y$, and 
...
called a stratification\index{stratification}.  In the case of polytopes such as the cube this is essentially the facet structure.   
