...
\begin{definition}
		Let $M$ and $N$ be smooth manifolds, $W\subseteq N$ a smooth submanifold, and $f:M\to N$ a smooth map. We say that $f$ is transversal to $W$ and write $f\pitchfork W$ if $\mathrm{im}(df_x)+T_{f(x)}W=T_{f(x)}N$ for all $x\in f^{-1}(W)$. Note: this sum will not be direct in general.
	\end{definition}
\begin{example}
	If $Z\subset N$ is another submanifold such that $Z\pitchfork W$ and $f:Z\hookrightarrow N$ is the inclusion map, then $f\pitchfork W$.
\end{example}
